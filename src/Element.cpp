...
#include <iostream>
#include <memory>
#define DENSITY 1000.f
#define PHI 1000.f //incompressibility for viscosity
#define PSI 1.f //rigidity for viscosity
#define LAMBDA 10.f //incompressibility
#define MU 1.f //rigidity
...
Vector3f Element::addForces(Matrix3f dx_du, Matrix3f stress, shared_ptr<Node> n1, std::vector<shared_ptr<Node>> nodes){
    ...
    normal.normalize();
    Vector3f centroid = (A+B+C)/3.f;
    Vector3f oppose = n1->m_position - centroid;
...
void Element::updateForces(){
    ...
    Matrix3f dx_du = P*m_beta.inverse();
    Matrix3f dx_dot_du = V*m_beta.inverse();
